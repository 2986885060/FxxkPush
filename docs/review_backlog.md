...
> **r10（2026-09-23）已全部清仓**：8 条留档全部修复（P2-7 更早已修），
> 行号已随分包重组整体漂移，定位一律按函数名/代码串。
...
- [x] **P2-1　告警内容定位性不足** — 原 `watchdog.py:609-614`
...
- [x] **P2-2　无跨检查项合并/去重** — 原 `watchdog.py:570-623`
...
- [x] **P2-3　微信窗口被最小化/关闭 → 采集静默停摆但三项全绿**
...
- [x] **P2-4　notification_listener 运行期 WinRT 权限被撤 → 返回空但心跳照打**
...
- [x] **P2-5　listener 的「看门人已死」告警只有单通道**
...
- [x] **P2-8　VPS `save_state` 非原子 + `_state` 形状无校验**
...
- [x] **P2-9　wechat 08:00 换挡的 quiet 余量只有约 2 分钟**
